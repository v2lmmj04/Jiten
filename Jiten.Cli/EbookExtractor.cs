...
public class EbookExtractor
{
    public async Task<string> ExtractTextFromEbook(string filePath)
    {
        ...
        // Select only the chapters with key partXXXX.html or p-XXX.html
        // I'm not sure if all epub files have this format, but it works for now
        Regex regex = new(@"\b(part\d{4}\.x?html|p-\d{3}\.x?html)\b");
        ...
        foreach (var chapter in filteredChapters)
        {
            ...
            if (nodes != null)
            {
                ...
                foreach (HtmlNode node in nodes)
                {
                    ...
                    else if (node.NodeType == HtmlNodeType.Text && !node.ParentNode.Name.Equals("rt", StringComparison.OrdinalIgnoreCase))
                    {
