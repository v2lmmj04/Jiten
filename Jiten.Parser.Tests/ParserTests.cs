...
public class ParserTests
{
    ...
    [Theory]
    [InlineData("ご注文はうさぎですか", new[] { "ご注文", "は", "うさぎ", "です", "か" })]
    ...
    [InlineData("しつれいしなければならないんです", new[] { "しつれいし", "なければならない", "ん", "です" })]
    ...
    [InlineData("ほんとは好きなんだと", new[] { "ほんと", "は", "好き", "な", "ん", "だ", "と" })]
    ...
    [InlineData("そう呼んでくれていい", new[] { "そう", "呼んでくれていい" })]
    [InlineData("払わなくてもいい", new[] { "払わなくてもいい" })]
    ...
    [InlineData("かわいいです", new[] { "かわいいです" })]
    [InlineData("学生なんだ", new[] { "学生", "な", "ん", "だ" })]
    [InlineData("なんだから", new[] { "な", "ん", "だから" })]
    [InlineData("名付けたい", new[] { "名付けたい" })]
    [InlineData("切なくなってしまう", new[] { "切なくなってしまう" })]
    [InlineData("らいかな", new[] { "らい", "かな" })]
    [InlineData("誰かいなくなった", new[] { "誰か", "いなくなった" })]
    [InlineData("思い出すな", new[] { "思い出す", "な" })]
    [InlineData("かなって思ったら", new[] { "かなって", "思ったら" })]
    [InlineData("法律にかなっているさま", new[] { "法律", "に", "かなっている", "さま" })]
    [InlineData("ことすら難しい", new[] { "こと", "すら", "難しい" })]
    [InlineData("投下しました", new[] { "投下しました" })]
    [InlineData("そんなのでいいと思ってるの", new[] { "そんな", "の", "でいい", "と", "思ってる", "の" })]
    [InlineData("だけが墓参りしてた", new[] { "だけ", "が", "墓参りしてた" })]
    [InlineData("はいいんだけどな", new[] { "は", "いい", "ん", "だけど", "な" })]
    [InlineData("なりつつあるんだが", new[] { "なりつつある", "ん", "だが" })]
    ...
    [InlineData("泣きそうなんだけど", new[] { "泣きそう", "な", "ん", "だけど" })]
    [InlineData("これですね", new[] { "これ", "です", "ね" })]
    [InlineData("はいなくなります", new[] { "は", "いなくなります" })]
    ...
    [InlineData("注意してください", new[] { "注意してください" })]
    [InlineData("昨日といいどうしてこう", new[] { "昨日", "と", "いい", "どうして", "こう" })]
    [InlineData("いっぱいきそう", new[] { "いっぱい", "きそう" })]
    [InlineData("仲良しになったら", new[] { "仲良し", "に", "なったら" })]
    [InlineData("全くといっていい", new[] { "全く", "と", "いって", "いい" })]
    [InlineData("発狂しそうなんだ", new[] { "発狂しそう", "な", "ん", "だ" })]
    ...
    [InlineData("おまえら低いんだよ", new[] { "おまえら", "低い", "ん", "だ", "よ" })]
    ...
    [InlineData("私にとっても", new[] { "私", "にとって", "も" })]
    [InlineData("睡眠を十分にとってください", new[] { "睡眠", "を", "十分", "に", "とってください" })]
    [InlineData("そうなんだけど", new[] { "そう", "な", "ん", "だけど" })]
    [InlineData("進んでない", new[] { "進んでない" })]
    [InlineData("一回だけであとは言わない", new[] { "一回", "だけ", "で", "あと", "は", "言わない" })]
    [InlineData("ご親切に恐縮しております", new[] { "ご親切に", "恐縮", "しております" })]
    ...
    [InlineData("アレハレ", new[] { "アレ" })]
    [InlineData("上に文字があったり", new[] { "上", "に", "文字", "が", "あったり" })]
    [InlineData("言っただろ", new[] { "言った", "だろ" })]
    [InlineData("嵐が起ころうとしている", new[] { "嵐", "が", "起ころうとしている" })]
    [InlineData("知らないでしょう", new[] { "知らないでしょう" })]
    [InlineData("読まないでしょう", new[] { "読まないでしょう" })]
    [InlineData("来ないでしょう", new[] { "来ないでしょう" })]
    ...
    [InlineData("安心させた", new[] { "安心させた" })]
    [InlineData("人が好きそうだ", new[] { "人", "が", "好き", "そう", "だ" })]
    [InlineData("もっていこうとする", new[] { "もっていこうとする" })]
    [InlineData("増やして", new[] { "増やして" })]
    [InlineData("ぜいたくで", new[] { "ぜいたく", "で" })]
    [InlineData("したくらいで", new[] { "したくらい", "で" })]
    ...
    [InlineData("私にとっては少しおかしいです", new[] { "私", "にとって", "は", "少し", "おかしいです" })]
    ...
    [InlineData("はいずれ", new[] { "は", "いずれ" })]
    [InlineData("お下がり", new[] { "お下がり" })]
    [InlineData("でも1000台とか1桁はあんまりだよな", new[] { "でも", "1000台", "とか", "1桁", "は", "あんまり", "だ", "よな" })]
    ...
    [InlineData("となったらしい", new[] { "となったらしい" })]
    ...
    [InlineData("堪能させていただきます", new[] { "堪能させていただきます" })]
    [InlineData("わからんからそう思った", new[] { "わからん", "から", "そう", "思った" })]
    [InlineData("うちからそうなっても", new[] { "うち", "から", "そう", "なっても" })]
    [InlineData("上映会やな", new[] { "上映", "会", "や", "な" })]
    [InlineData("以上書いてください", new[] { "以上", "書いてください" })]
    ...
    [InlineData("知れないですか", new[] { "知れないです", "か" })]
    [InlineData("かも知れないですね", new[] { "かも知れない", "です", "ね" })]
    [InlineData("匙ですくう", new[] { "匙", "で", "すくう" })]
    [InlineData("デカかったクドくない", new[] { "デカかった", "クドくない" })]
    [InlineData("決めたらしい教われたらしい", new[] { "決めたらしい", "教われたらしい" })]
    [InlineData("臆病なくせにとてもよい仲間だった", new[] { "臆病", "な", "くせに", "とても", "よい", "仲間", "だった" })]
    ...
    [InlineData("天をうらんでみたところで始まらない", new[] { "天", "を", "うらんで", "みた", "ところで", "始まらない" })]
    [InlineData("癒やされたかった", new[] { "癒やされたかった" })]
    [InlineData("7時には帰ってきなさい", new[] { "7時", "には", "帰ってきなさい" })]
    [InlineData("人はいますか", new[] { "人", "は", "います", "か" })]
    [InlineData("トマトづくし", new[] { "トマト", "づくし" })]
    [InlineData("見えざる関係性", new[] { "見えざる", "関係性" })]
    [InlineData("だめだったら", new[] { "だめ", "だったら" })]
    [InlineData("万事不都合の無いようにはからってくれ", new[] { "万事", "不都合", "の", "無い", "ように", "はからってくれ" })]
    ...
    [InlineData("エレガントなお洋服", new[] { "エレガント", "な", "お", "洋服" })]
    [InlineData("老いてなお元気なこと", new[] { "老いて", "なお", "元気", "な", "こと" })]
    ...
    [InlineData("折りたたみ式ついたて", new[] { "折りたたみ", "式", "ついたて" })]
    [InlineData("いろいろな部分をもんだりこすったりすること", new[] { "いろいろ", "な", "部分", "を", "もんだり", "こすったり", "する", "こと" })]
    [InlineData("たまにはいいもんだよ", new[] { "たまに", "は", "いい", "もんだ", "よ" })]
    ...
    [InlineData("長くはかからないでしょう", new[] { "長く", "は", "かからないでしょう" })]
    [InlineData("人はいないでしょうね", new[] { "人", "は", "いないでしょう", "ね" })]
    [InlineData("人はいないですね", new[] { "人", "は", "いないです", "ね" })]
    ...
    [InlineData("語れるもんだな", new[] { "語れる", "もんだ", "な" })]
    [InlineData("筋をもんでくれ", new[] { "筋", "を", "もんでくれ" })]
    ...
    [InlineData("30分後におかけ直しください", new[] { "30分", "後", "に", "お", "かけ直し", "ください" })]
    ...
    [InlineData("がんもどきという食品", new[] { "がんもどき", "と", "いう", "食品" })]
    [InlineData("落ちこぼれている", new[] { "落ちこぼれている" })]
    [InlineData("1話しか見てない", new[] { "1話", "しか", "見てない" })]
    ...
    [InlineData("人口10万人以上の都市の中で唯一旅客を扱う鉄道駅が存在せず",
                new[] { "人口", "10万人", "以上", "の", "都市", "の", "中", "で", "唯一", "旅客", "を", "扱う", "鉄道駅", "が", "存在", "せず" })]
    [InlineData("だし", new[] { "だ", "し" })]
