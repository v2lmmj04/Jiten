...
// Retrieve the deck
        ...
        foreach (var wf in list)
        {
            Console.WriteLine($"Percentage of words in rank {wf.Key} : {wf.Value / (double)words.Count * 100}");
            Console.WriteLine("Points: " + (wf.Value / (double)words.Count * 100) * Math.Pow(wf.Key, wf.Key));
        }
    }
}